...
_MDNS_PORT = 5353
_DNS_PORT = 53
_MDNS_SOCKADDR_IPV6 = (_MDNS_ADDR_IPV6, _MDNS_PORT, socket.AF_INET6, socket.SOCK_DGRAM)
_MDNS_SOCKADDR_IPV4 = (_MDNS_ADDR, _MDNS_PORT, socket.AF_INET, socket.SOCK_DGRAM)
...
class DNSAddress(DNSRecord):
    ...
    def __init__(self, name, type_, class_, ttl, address, interface=""):
    ...
    def __repr__(self):
        """String representation"""
        try:
            if 4 == ip_kind(self.address):
                return socket.inet_ntoa(self.address)
            elif 6 == ip_kind(self.address):
                return socket.inet_ntop(self.address)
...
class DNSIncoming(object):
    ...
    def __init__(self, data, sockaddr):
    ...
    def read_interface(self, sockaddr):
        """Reads interface from sockaddr of packet"""
        src = sockaddr[0]
    ...
    def read_others(self):
        ...
        for i in xrange(n):
            ...
            if type_ == _TYPE_A:
                ...
                rec = DNSAddress(
                    domain, type_, class_, ttl, self.read_string(16), self.interface)
            else:
...
class Listener(object):
    ...
    def handle_read(self, socket_):
        data, sockaddr = socket_.recvfrom(_MAX_MSG_ABSOLUTE)
        log.debug('Received %r from %r:%r', data, sockaddr[0], sockaddr[1])
        (family, _, _, _, sockaddr) = socket.getaddrinfo(sockaddr[0], sockaddr[1])[0]
        (host, port) = sockaddr[0], sockaddr[1] #allow ipv4 addresses to also ignore flowinfo and scopeid
        if socket.AF_INET6 == family:
            mdns_addrinfo = socket.getaddrinfo(*_MDNS_SOCKADDR_IPV6)
        else:
            mdns_addrinfo = socket.getaddrinfo(*_MDNS_SOCKADDR_IPV4)
        
        (_,_,_,_,mdns_sockaddr) = mdns_addrinfo[0]
        
        self.data = data
        msg = DNSIncoming(data, sockaddr)
        if msg.is_query():
            (_,port) = socket.getnameinfo(sockaddr, socket.AI_NUMERICHOST)
            # Always multicast responses
            #
            if _MDNS_PORT == port:
                self.zc.handle_query(msg, mdns_sockaddr)
            # If it's not a multicast query, reply via unicast
            # and multicast
            #
            elif _DNS_PORT == port:
                self.zc.handle_query(msg, sockaddr)
                self.zc.handle_query(msg, mdns_sockaddr)
        else:
    ...
    def run(self):
        ...
        while True:
            ...
            if self.next_time <= now:
                ...
                mdns_addrinfo = socket.getaddrinfo(*_MDNS_SOCKADDR_IPV6)
                (_,_,_,_,mdns_sockaddr) = mdns_addrinfo[0]
                self.zc.send(out, mdns_sockaddr)
                mdns_addrinfo = socket.getaddrinfo(*_MDNS_SOCKADDR_IPV4)
                (_,_,_,_,mdns_sockaddr) = mdns_addrinfo[0]
                self.zc.send(out, mdns_sockaddr)
    ...
    def __repr__(self):
        ...
        return '%s(%s)' % (
            ...
            ', '.join(
                ...
                for name in (
                    'type', 'name', 'address', 'interface', 'port', 'weight', 'priority',
                    'server', 'properties',
                )
            )
        )
...
def new_socket(ip_version = socket.AF_INET6, address = None, port = None):
    s = socket.socket(ip_version, socket.SOCK_DGRAM)
    ...
    loop = struct.pack('@i', 1)
    
    if socket.AF_INET6 == ip_version:
        s.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_MULTICAST_HOPS, ttl)
        s.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_MULTICAST_LOOP, loop)
    else:
    ...
    if None != port:
        (family, type, proto, canonname, sockaddr) = socket.getaddrinfo(address, port, ip_version, socket.SOCK_DGRAM)[0]
        s.bind(sockaddr)
    return s
...
class Zeroconf(object):
    ...
    def __init__(
        ...
        self._listen_socket_ipv4 = new_socket(socket.AF_INET, '0.0.0.0',_MDNS_PORT)
        self._listen_socket_ipv6 = new_socket(socket.AF_INET6, '::',_MDNS_PORT)
        ...
        #interfaces_ipv6 = normalize_interface_choice(interfaces, socket.AF_INET6)
        for i in socket.if_nameindex():
            respond_socket = new_socket(socket.AF_INET6, '::',_MDNS_PORT)
            respond_socket.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_MULTICAST_IF,
                struct.pack('I', i[0]))
            respond_socket.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_JOIN_GROUP,
                socket.inet_pton(socket.AF_INET6, _MDNS_ADDR_IPV6) + struct.pack('I', i[0])) 
            self._respond_sockets.append(respond_socket)
        ### IPv4
        for i in interfaces:
            log.debug('Adding %r to multicast group', i)
            try:               
                self._listen_socket_ipv4.setsockopt(
            ...
            respond_socket = new_socket(socket.AF_INET, '0.0.0.0',_MDNS_PORT)
        ...
        self.engine.add_reader(self.listener, self._listen_socket_ipv4)
        self.engine.add_reader(self.listener, self._listen_socket_ipv6)
    ...
    def handle_query(self, msg, sockaddr):
        ...
        for question in msg.questions:
            if question.type == _TYPE_PTR:
                ...
                try:
                    ...
                    if question.type in (_TYPE_AAAA, _TYPE_ANY):
                        for service in self.services.values():
                            if service.server == question.name.lower():
                                out.add_answer(msg, DNSAddress(
        ...
        if out is not None and out.answers:
            out.id = msg.id
            self.send(out, sockaddr)

    def send(self, out, sockaddr):
        """Sends an outgoing packet."""
        packet = out.packet()
        log.debug('Sending %r as %r...', out, packet)
        family = socket.getaddrinfo(*sockaddr[0:2])[0][0]
        for s in self._respond_sockets:
            ...
            if self._GLOBAL_DONE:
                return
            try:
                bytes_sent = s.sendto(packet, 0, sockaddr[0:2])
    ...
    def close(self):
        ...
        if not self._GLOBAL_DONE:
            ...
            self.engine.del_reader(self._listen_socket_ipv4)
            self.engine.del_reader(self._listen_socket_ipv6)
            self._listen_socket_ipv4.close()
            self._listen_socket_ipv6.close()
            self.engine.join()
